// Neural appearance model residual chunk (spliced into capture_common.glsl at
// its `// @inject residual_eval` marker; the wrapper provides the uniforms,
// the color activation, and the captureColor entry point).
//
// The residual MLP is evaluated as in SMERF: the (bias-free) weights are
// indexed as texels — one texel = 4 consecutive input weights of one output
// neuron — and each hidden layer accumulates `input_block * mat4(4 texels)`
// per 4-wide output block; the 3-row output layer reads 3 texels per input
// block. Texel order:
...
// pre-multiplied into the per-splat h0_static cache, 0 = the legacy layout
// where the per-splat textures hold the encoded features and layer 0 is the
// full trained matrix), N_L0_IN (layer-0 input width as stored in the file),
// N_PARAM_TEX (1 or 2 per-splat fp16 textures), N_FEATURES / SH0_INPUT
// (legacy layout only), N_NEURONS, N_HIDDEN, SH_DEGREE_MASK, RESIDUAL_ACT
// (0 none / 1 tanh / 2 softplus(β10); the loader resolves the legacy additive
// combo's implied softplus into code 2), D1_OFF..D6_OFF (layer-0-input-array
// offsets of the view-direction SH degrees).
//
// Color model (matches the reference implementation; the wrapper composes
// color_activation(base + residualColor(...))):
