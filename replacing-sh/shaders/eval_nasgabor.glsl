...
// at its `// @inject residual_eval` marker; the wrapper provides the uniforms,
// the color activation, and the captureColor entry point).
//
// Normalized Anisotropic Spherical Gaussians with a Gabor term:
//     R(dir) = Σ_l pdf_l(dir) * weight_l
// with the per-lobe response, given the lobe frame vectors x and z,
//     K   = (<v, z> + 1) / 2
//     K_e = eps + a * <v, x>² / (1 - <v, z>²)
//     E   = K^K_e
//     G   = (1 + cos(k * <v, x>)) / 2                         [Gabor term]
//     pdf = exp(2λ (E K - 1)) * E * G * N
// Everything view-independent is baked at export time: the per-splat payload
// holds N_LOBES lobes of 13 fp16 values each — the frame vectors x and z
// (from the tanh-activated frame angles), 2λ, the anisotropy a, the Gabor
// frequency k, the normalization constant N = λ√(1+a) / (2π(1+ε−e^{−2λ})),
// and the activated rgb weights — so the per-frame work is just the
// direction-dependent response. At the +z pole the pdf is the constant 1 (no
// normalization), at the −z pole 0, exactly like the reference renderer.
//
