// Shared scaffolding of the capture-pass color evaluation. main.js splices
// the active appearance model's residual chunk (eval_sh / eval_sv / eval_nasg
// / eval_nasgabor / eval_neural.glsl) into the marker below and injects the
// result into capture.frag and capture.vert — some Android drivers return
// constants from fragment-stage integer texelFetch, so the same code must run
// in either stage (see main.js chooseCaptureStage). Each chunk defines
//     vec3 residualColor(ivec2 texel, vec3 dir)
// — the view-dependent color residual of the splat at texture coordinate
// `texel` seen from unit direction `dir` — and this file provides the shared
// uniforms and helpers plus the captureColor() entry point composing
// color_activation(base + residual).
//
// Injected defines used here: N_PARAM_TEX (number of per-splat parameter
// textures), COLOR_ACT (0 relu / 1 softplus(β10) / 2 sigmoid(4x) / 3 satexp /
// 4 hardsigmoid / 5 none), BASE_ACT (0 identity / 1 exp(3x)), COLOR_SHIFT
// (the 0.5 gray shift of the shifted color activations when it could not be
// baked into the stored base — i.e. with the exp base — and 0.0 otherwise),
// RENDER_BASE / RENDER_RESIDUAL (the reference renderer's base-color and
// residual debug switches).

uniform highp usampler2D splatData;
// per-splat residual parameters; the layout is appearance-model specific (fp16
// value streams for the neural/voronoi/lobe models, bit-packed quantized
// coefficient levels for the sh model — see the chunks)
#if N_PARAM_TEX > 0
...
// color activation over base + residual. The 0.5 shift of the shifted
// variants is pre-baked into the stored base with the identity base
// activation, so COLOR_SHIFT is 0.0 there; with the exp base it is applied
// here (sigmoid(4x) and satexp are unshifted by definition)
...
#if N_PARAM_TEX > 0
// Unpacks the splat's fp16 parameter stream (8 values per RGBA32UI texel) into
// a flat array — used by the chunks whose payload is a plain value stream (the
// sh chunk instead decodes its bit-packed texels directly and never calls this,
// so it compiles away there).
...
vec4 captureColor(ivec2 texel) {
    ...
    // (both the sorter's count and the vertex shader), so skip the residual for
    // them. The base-only fallback only shows if the one-frame-late sort order
    // still references such a splat — where the stale ordering dominates any
    // color error.
